...
# Order by A → Z
voter_party_only[, last_name := toupper(trimws(last_name))]
setorder(voter_party_only, last_name)


voter_party_only[, last_initial := substr(last_name, 1, 1)]

# Divide
voter_party_A_M <- voter_party_only[last_initial <= "M"][, !"last_initial"]
voter_party_N_Z <- voter_party_only[last_initial >= "N"][, !"last_initial"]

# Check sizes
nrow(voter_party_A_M)
nrow(voter_party_N_Z)


write_xlsx(voter_party_A_M, "party_voter_change_A_M.xlsx")
write_xlsx(voter_party_N_Z, "party_voter_change_N_Z.xlsx")
```
